...
from ups_sythn import pms_transforms
from . import util
np.random.seed(0)

class UpsSynthTestDataset(data.Dataset):
    def __init__(self, syn_obj, light_index):
        bm_dir = 'data/PS_Synth_MERL_Test_ECCV2020'
        self.root = os.path.join(bm_dir)
        split = 'test'
        self.split = 'test'

        self.syn_obj = syn_obj
        self.light_index = light_index
        self.int_aug = True  # and not no_int_aug:
        """
        self.args.test_resc = False
        self.args.test_h = 128
        self.args.test_w = 128
        """
        self.shape_list = util.read_list(os.path.join(self.root, '%s_mtrl_%s.txt' % 
            (split, self.syn_obj)), sort=False)
        self.repeat = 1
    ...
    def _get_input_path(self, index):
        ...
        if self.light_index != None:
            index_path = os.path.join(self.root, 'Lighting_Index', self.light_index)
    ...
    def __getitem__(self, index):
        ...
        if self.int_aug: # and not no_int_aug:
        ...
        item['dirs'] = torch.from_numpy(dirs).float()
        item['ints'] = torch.from_numpy(ints).float()
        ...
        item['img'] = item['img'].view(-1, 3, h, w)
        return item
...
if __name__ == '__main__':
    syn_obj = 'dragon'
    light_index = 'light_82_idx.txt'
    a = UpsSynthTestDataset(syn_obj, light_index)
    a1 = a.__getitem__(0)
